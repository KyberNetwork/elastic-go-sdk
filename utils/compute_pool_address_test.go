...
func TestComputePoolAddress(t *testing.T) {
	...
	result, err := ComputePoolAddress(factoryAddress, tokenA, tokenB, constants.Fee001, "")
	if err != nil {
		panic(err)
	}
	...
	resultA, err := ComputePoolAddress(factoryAddress, USDC, DAI, constants.Fee001, "")
	if err != nil {
		panic(err)
	}
	resultB, err := ComputePoolAddress(factoryAddress, DAI, USDC, constants.Fee001, "")
	if err != nil {
		panic(err)
	}
